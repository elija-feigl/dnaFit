...
class Cascade:
    ...
    def run_cascaded_fitting(
        self, base_time_steps: int, resolution: float, is_sr=False, is_film=False, include_ss=False
    ):
        ...
        def _regular(step, folder):
            ...
            previous_folder = self._run_namd(folder=folder, namd_file=namd_file)
            step += 1
        ...
        def _annealing_incr(step, folder, gscale=0.01):
            ...
            time_steps_last = create_namd_file(namd_file, ts=ts_relax, i_temp=300, f_temp=400)
            self.logger.debug(
        ...
        def _annealing_decr(step, folder, gscale=0.01):
            ...
            namd_file = Path(f"run/{prefix}_c-mrdna-MDff-{step}.namd")
            time_steps_last = create_namd_file(namd_file, ts=ts_relax, i_temp=400, f_temp=300)
            self.logger.debug(
        ...
        if not Path(f"run/{grid_pdb}").exists():
            ...
            linker.write_custom_gridpdb(dest=Path(f"run/{grid_pdb}"), exclude_ss=(not include_ss))
        ...
        if not self.generated_with_mrdna:  # pure enrgMD run without map
            ...
            time_steps_last = create_namd_file(namd_file, ts=ts_enrg, ms=ms_enrg, mdff="0")
            self.logger.debug(f"{folder}: ts={ts_enrg}, ms={ms_enrg}, mdff=0")
            previous_folder = self._run_namd(folder=folder, namd_file=namd_file)
            init = 0
        ...
        for cascade in range(n_repeat):
            for n in range(n_cascade):
                ...
                if cascade == 1 and n == 0:
                    ...
                    grid_file = "grid-0.dx"
                    folder = f"{step}"
                    time_steps_last, previous_folder, step = _annealing_incr(step, folder)
                    folder = f"{step}"
                    time_steps_last, previous_folder, step = _annealing_decr(step, folder)
        ...
        if not final_conf.is_file():
            self.logger.error(f"cascaded fit incomplete. {final_conf} not found ")
            sys.exit(1)
    ...
    def _vmd_prep(self, n_cascade, resolution):
        ...
        for n in range(n_cascade):
            gfilter = (n * (resolution - RES_SPAN)) / n_layers + RES_SPAN - resolution
            lines.append(f"volutil -smooth {gfilter} run/base.dx -o run/{n}.dx")
    ...
    def _vmd_post(self, step):
        ...
        if Path("./run/enrgMD").is_dir():
            lines.append(f"mol addfile ./run/enrgMD/{name}.dcd start 0 step 1 waitfor all")
        for n in range(step):
            lines.append(f"mol addfile ./run/{n}/{name}.dcd start 0 step 1 waitfor all")

...
def get_wc(universe, segid, resid, Hbond_dev=0.1):
    ...
    try:
        ...
        res = seg.atoms[seg.atoms.resids == resid].residues[0] #residues returns a list of objects -> her just one entry
    except IndexError:
    ...
    dist_C1p = mda.analysis.distances.distance_array(C1p.positions, C1p_compl.positions)
    idx_C1p_close = np.argwhere(abs(dist_C1p - C1P_BASEDIST) < C1P_BASEDIST*Hbond_dev)
    res_close = C1p_compl[idx_C1p_close[:,1]].residues #candidates
    
    #check candidates for base-complementary
    res_candidates = res_close[np.where(res_close.resnames == WC_DICT[res.resname])]

    #check candidates for WC-Hbonds. due to numbering 5'-> 3' its probably best to search cadidates in regular direction
    hbond_res = res.atoms.select_atoms("name " + ' or name '.join(map(str, WC_HBONDS[res.resname] )))
    ...
    for res_can in res_candidates:
        ...
        badness_break = np.inf #overwritten anyways, high to allow logic (min)
        badness_non = 0. #overwritten anyways, low to allow logic (max)
        
        hbond_res_can = res_can.atoms.select_atoms("name " + ' or name '.join(map(str, WC_HBONDS[res_can.resname] )))
        
        for idx, hbond_atom in enumerate(hbond_res_can):
            hbond_dist = mdamath.norm(hbond_res[idx].position - hbond_atom.position)
            hbond_should = WC_HBONDS_DIST[res.resname][idx]

            if abs(hbond_dist - hbond_should) < hbond_should *Hbond_dev :
                is_partner = True
                badness = max(badness, abs(hbond_dist - hbond_should) / (hbond_should))
            else:
                is_partner = False
                badness_non = max(badness_non, abs(hbond_dist - hbond_should) / (hbond_should))
...
def get_wc_dict(universe, Hbond_dev=0.1):
    ...
    for res in universe.residues:
        ...
        segid_wc, resid_wc, badness = get_wc(universe, segid, resid, Hbond_dev=Hbond_dev)
        resindex_wc = None
        
        if segid_wc is not None:
            res_wc = universe.atoms.select_atoms('segid {0!s} and resid {1!s}'.format(segid_wc,resid_wc)).residues[0]
...
def main():
    ...
    for dev in deviations:
        ...
        pickle.dump( (u, wc_pairs, wc_index_pairs), open( output + "__wc_pairs-" + dev +".p", "wb" ) )
